...
class ProductRecommender(object):
    ...
    def fit(self, user_rating, latent_features_guess=2, learning_rate=0.0002, steps=5000,
            regularization_penalty=0.02, convergeance_threshold=0.001, start_step=0):
        """
        ...
        print(f'start at step {start_step} to {steps}')
        return self.__factor_matrix(user_rating, latent_features_guess, learning_rate, steps, regularization_penalty,
                                    convergeance_threshold, start_step)
    ...
    def __factor_matrix(self, R, K, alpha, steps, beta, error_limit, start_step):
        """
        ...
        for step in range(start_step, steps):
            ...
            print('step : ', step, ' cost = ', error, ' dif :', temp - error)
            if step % 100 == 0:
                print('Saving P and Q')
                numpy.save(f'checkpoint/user_matrix/P_{step}.npy', P)
                numpy.save(f'checkpoint/movie_matrix/Q_{step}.npy', Q)
    ...
    def __print_fit_stats(self, error, samples_count, products_count):
        ...
        print('Movies: ' + str(products_count))
        print('------------------------------')
